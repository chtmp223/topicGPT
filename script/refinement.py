...
def topic_pairs(topic_sent, all_pairs, threshold=0.5, num_pair=2):
    ...
    while count < num_pair and idx < len(pairs): 
        ...
        if float(pairs[idx]['score']) > threshold:
            if (sorted([topic_sent[i], topic_sent[j]]) not in all_pairs) and (topic_sent[i] != topic_sent[j]):
...
def merge_topics(topics_root, topics_node, prompt, deployment_name, temperature, max_tokens, top_p, verbose): 
    '''
    ...
    topic_sent = [f"[{topic.lvl}] {topic.name}: {topic.desc}" for topic in topics_root.descendants]
    labels = [f"[{topic.lvl}] {topic.name}" for topic in topics_root.descendants]
    new_pairs, all_pairs = topic_pairs(topic_sent, all_pairs=[], threshold=0.5, num_pair=2)     

    responses, removed, orig_new = [], [], {}
    # Pattern to match generations
    top_pattern = regex.compile("^\[(\d+)\]([\w\s\-\',]+)(?:[:\(\)\w\s\/])*?:([\w\s,\.\-\/;']+) \(((?:\[\d+\] [\w\s\-\',]+(?:, )*)+)\)$")
    ...
    while len(new_pairs) > 1:             
        ...
        if verbose: print(refiner_input)

        try: 
            input_len = num_tokens_from_messages(refiner_input, 'gpt-4')
            response = api_call(refiner_prompt, deployment_name, temperature, max_tokens, top_p)
            responses.append(response)
            merges = response.split('\n')
            for merge in merges: 
                match = regex.match(regex.compile(top_pattern), merge.strip())
                if match:
                    lvl, name, desc = int(match.group(1)), match.group(2).strip(), match.group(3).strip()
                    origs = [t.strip(", ") for t in regex.findall(orig_pattern, match.group(4).strip())]
                    orig_count = 0
                    add = False
                    if len(origs) > 1:
                        for node in topics_root.descendants:
                            if f"[{node.lvl}] {node.name}" in origs and f"[{node.lvl}] {node.name}" != f"[{lvl}] {name}":
                                orig_new[f"[{node.lvl}] {node.name}:"] = f"[{lvl}] {name}:"
                                if f"[{node.lvl}] {node.name}: {node.desc}" in topic_sent: 
                                    if verbose: print(f"Removing [{node.lvl}] {node.name}: {node.desc}\n")
                                    topic_sent.remove(f"[{node.lvl}] {node.name}: {node.desc}")
                        ...
                        if add and f"[{lvl}] {name}" not in removed: 
                            if f"[{lvl}] {name}: {desc}" not in topic_sent and f"[{lvl}] {name}" not in labels: 
                                new_node = Node(parent=topics_root, lvl=lvl, name=name, desc=desc, count=orig_count)
                                if verbose: print(f"Adding [{lvl}] {name}: {desc}\n")
                                topic_sent.append(f"[{lvl}] {name}: {desc}")
                                topics_node.append(new_node)
                            else:
                                if verbose: print(f"[{lvl}] {name} already exists!\n")
                                for node in topics_root.descendants:
                                    if f"[{node.lvl}] {node.name}" == f"[{lvl}] {name}":
...
def remove_topics(topics_root, verbose, threshold=0.01):
    ...
    for node in topics_root.descendants: 
        if node.count < threshold and node.lvl==1: 
            if verbose: print(f"Removing {node.name} ({node.count} counts)")
            node.parent = None
    return topics_root
...
def main(): 
    ...
    parser.add_argument("--prompt_file", type=str, default="prompt/refinement.txt", help="file to read prompts from")
    parser.add_argument("--generation_file", type=str, default="data/output/generation_1.jsonl", help="file to read generation results from")
    parser.add_argument("--topic_file", type=str, default="data/output/generation_1.md", help="file to read topics from")
    parser.add_argument("--out_file", type=str, default="data/output/refinement_1.md", help="file to write merged topics to")
    parser.add_argument("--updated_file", type=str, default="data/output/refinement_1.jsonl", help="file to update merged topics to")
    parser.add_argument("--verbose", type=bool, default=False, help="whether to print out results")
    parser.add_argument("--remove", type=bool, default=False, help="option to remove minor topics")
    parser.add_argument("--mapping_file", type=str, default="data/output/refiner_mapping.json", help="file to write refined mapping to")
    parser.add_argument("--refined_again", type=str, default="refiner", help="Is this the second time refining?")

    args = parser.parse_args()

    # Model configuration ----
    deployment_name, max_tokens, temperature, top_p = args.deployment_name, args.max_tokens, args.temperature, args.top_p
    if deployment_name == "gpt-35-turbo": deployment_name = "gpt-3.5-turbo"
    ...
    responses, updated_topics_root, mapping = merge_topics(topics_root, topics_node, refinement_prompt, 
                                       deployment_name, temperature, max_tokens, 
                                       top_p, args.verbose)
    if args.remove: 
        updated_topics_root = remove_topics(updated_topics_root, 
                                            args.verbose, 
                                            threshold=0.01)
    ...
    if len(responses) > 0: 
        ...
        try: 
            ...
            if args.verbose: print(f"Writing to existing mapping file {args.mapping_file}")
        except:
            current = []
            ...
            for s in splitted: 
                for key, value in mapping.items():
                    if key != value and s.startswith(key):
                        ...
                        if args.verbose: print(f"Replacing {key} with {value}")
                sub_list.append(s)
