        ...
        if res in ["Error", "None"] or len(topics) == 0: 
            ...
            for top in topics: 
                ...
                if first in names: 
                    ...
                    if verbose: print("Hallucinated:", first)
                    a.append("NA")
...
def correct_topics(topics_root, df, errors,correction_prompt, deployment_name, context_len, temperature, top_p, max_tokens, verbose, max_top_len=1700):
    '''
    ...
    for i in tqdm(errors): 
        ...
        max_doc_len = context_len - num_tokens_from_messages(correction_prompt, deployment_name) - num_tokens_from_messages(seed_str, deployment_name)
...
def main(): 
    ...
    # Prompting ----
    df = correct_topics(topics_root, df, error+hallucinated, correction_prompt, deployment_name, context_len, temperature, top_p, max_tokens, args.verbose)
